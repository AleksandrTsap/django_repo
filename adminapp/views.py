...
from adminapp.forms import AdminShopUpdateUserForm, ProductCategoryCreateForm, AdminProductUpdateForm
...
class ShopUserAdminUpdate(SuperUserOnlyMixin, PageTitleMixin, UpdateView):
    ...
    form_class = AdminShopUpdateUserForm
    success_url = reverse_lazy('new_admin:index')
    pk_url_kwarg = 'user_pk'
    page_title = 'админка/юзер/редактирование'
...
class ProductCategoryCreate(SuperUserOnlyMixin, PageTitleMixin, CreateView):
    ...
    page_title = 'админка/редактирование'


class ProductCategoryUpdate(PageTitleMixin, UpdateView):
    model = ProductCategory
    form_class = ProductCategoryCreateForm
    success_url = reverse_lazy('new_admin:categories')
...
def category_products(request, pk):
    ...
    context = {
        'page_title': f'категория {category.name}/продуктры',
        'category': category,
